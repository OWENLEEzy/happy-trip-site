(function(){
  ...
  function renderLink(link, className = 'lnk'){
    ...
    const priority = link.priority || 'primary';
    return `
      <a class="${className} ${type}" data-priority="${attr(priority)}" href="${attr(link.url)}" target="_blank" rel="noopener noreferrer" aria-label="${attr(label)}">
        ${ICONS[type]}
        <span>${esc(label)}</span>
        ${ICONS.arr}
      </a>
  ...
  function sectionHasBody(section){
    // Item sections render points/note/image only; `steps` are a generalResources
    // feature (renderResourceSteps), so a steps-only item section has no body here.
    return (Array.isArray(section.points) && section.points.length) ||
      !!section.note || !!section.image;
  }

  function renderItemSections(sections){
    if (!Array.isArray(sections)) return '';
    const filled = sections.filter(sectionHasBody);
    if (!filled.length) return '';
    return `
      <div class="it-sections">
        ${filled.map(section => `
          <section class="it-section">
            <div class="it-section-head">
              ${section.kicker ? `<span>${esc(section.kicker)}</span>` : ''}
              ${section.title ? `<h4>${esc(section.title)}</h4>` : ''}
            </div>
            ${section.image ? renderImage(section.image, 'section-media', section.title || '') : ''}
            ${Array.isArray(section.points) && section.points.length ? `
              <ul class="it-section-points">
                ${section.points.map(point => `<li>${esc(point)}</li>`).join('')}
              </ul>
            ` : ''}
            ${section.note ? `<p class="it-section-note">${esc(section.note)}</p>` : ''}
          </section>
        `).join('')}
      </div>
    `;
