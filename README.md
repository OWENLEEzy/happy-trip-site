...
**🌏 在线示例 / Live demo:** https://owenleezy.github.io/happy-trip-site/
（9 天马来西亚 + 新加坡行程 / a 9-day Malaysia & Singapore itinerary，位于 [`docs/`](docs/)）
...
整个技能就是 **`skill/happy-trip-site`** 这一个文件夹（入口是它里面的 `SKILL.md`）。安装就是把这个文件夹复制进 agent 的 skills 目录，**让文件夹名成为技能名**——复制后的路径必须以 `.../skills/happy-trip-site` 结尾，且 `SKILL.md` 就在它里面。

| Agent | 复制到这里 | 发现方式 |
|-------|-----------|---------|
| Claude Code | `~/.claude/skills/happy-trip-site` | 下次会话自动发现，用 `/happy-trip-site` 调用 |
| Codex | `~/.codex/skills/happy-trip-site` | 从 `~/.codex/skills` 自动发现 |
| 其他 agent | 任意你掌控的路径 | 让 agent 指向 `SKILL.md`，它按需加载其余文件 |

```bash
# Claude Code
cp -R skill/happy-trip-site ~/.claude/skills/happy-trip-site
# 验证落在正确路径（应看到一层之内就有 SKILL.md）
ls ~/.claude/skills/happy-trip-site/SKILL.md
```

⚠️ 常见错误：复制了仓库根目录，或多嵌了一层（`.../happy-trip-site/happy-trip-site/SKILL.md`）。只有当 `SKILL.md` **直接**位于 `.../skills/happy-trip-site/` 内时，agent 才能找到它。
...
The whole skill is the single folder **`skill/happy-trip-site`** (entrypoint `SKILL.md` inside it). Install it by copying that folder into the agent's skills directory **so the folder name becomes the skill name** — the copied path must end in `.../skills/happy-trip-site`, with `SKILL.md` directly inside.

| Agent | Copy the folder to here | Discovery |
|-------|-------------------------|-----------|
| Claude Code | `~/.claude/skills/happy-trip-site` | Auto-discovered next session; invoke with `/happy-trip-site` |
| Codex | `~/.codex/skills/happy-trip-site` | Auto-discovered from `~/.codex/skills` |
| Other agents | any path you control | Point the agent at `SKILL.md`; it loads referenced files on demand |

```bash
# Claude Code
cp -R skill/happy-trip-site ~/.claude/skills/happy-trip-site
# Verify it landed correctly (SKILL.md one level in)
ls ~/.claude/skills/happy-trip-site/SKILL.md
```

⚠️ Common mistake: copying the repo root, or nesting an extra folder (`.../happy-trip-site/happy-trip-site/SKILL.md`). The agent only finds the skill when `SKILL.md` sits **directly** inside `.../skills/happy-trip-site/`.
